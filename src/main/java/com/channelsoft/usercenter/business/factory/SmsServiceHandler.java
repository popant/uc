...
import com.channelsoft.usercenter.message.util.ApiKeyGenerator;

/**
 * <dl>
 * <dt>SmsServiceHandler</dt>
 * <dd>Description:短信服务处理类</dd>
 ...
 * @author cy
 */
@Component("smsServiceHandler")
public class SmsServiceHandler implements ProductHandler {
	...
	//后面需要附加上产品id,例如:/business/commercial/{productId}
	private String COMMERCIAL_URL = "/business/commercial";
	
	private String DEPOSIT_URL = "/purchase/gotoPurchase";
	//后面还需要加上企业id和产品id,例如:/agreement/display/{productId}
	private String AGREEMENT_URL = "/agreement/display";
	
	private Short productId = 3;

	public SmsServiceHandler(Short productId) {
		super();
	...
	/* (non-Javadoc)
	 * @see com.channelsoft.usercenter.business.factory.ProductHandler#getRemainTime()
	 */
	public Integer getRemainTime() throws HandlerException {
		String ret = null;
		Integer status = null;
		try {
			logger.info("进入SmsServiceHandler.getRemainTime()");
	...
	/* (non-Javadoc)
	 * @see com.channelsoft.usercenter.business.factory.ProductHandler#getEntStatus()
	 */
	public Integer getEntStatus() throws HandlerException {
		String ret = null;
		Integer status = null;
		try {
			logger.info("进入SmsServiceHandler.getEntStatus()");
			HttpClient client = HttpClientBuilder.create().build();
	...
	/* (non-Javadoc)
	 * @see com.channelsoft.usercenter.business.factory.ProductHandler#registerProduct()
	 */
	public String registerProduct() throws HandlerException {
		String ret = null;
		try {
			logger.info("进入SmsServiceHandler.registerProduct()");
			HttpClient client = HttpClientBuilder.create().build();
	...
	/* (non-Javadoc)
	 * @see com.channelsoft.usercenter.business.factory.ProductHandler#forCommercial()
	 */
	public String forCommercial() throws HandlerException {
		String ret = null;
		try {
			logger.info("进入SmsServiceHandler.forCommercial()");
			HttpClient client = HttpClientBuilder.create().build();
	...
	/* (non-Javadoc)
	 * @see com.channelsoft.usercenter.business.factory.ProductHandler#getAgentNum()
	 */
	public Integer getAgentNum() throws HandlerException {
		logger.info("进入SmsServiceHandler.getAgentNum()");
		return null;
	}

	/* (non-Javadoc)
	 * @see com.channelsoft.usercenter.business.factory.ProductHandler#openBusiness(com.channelsoft.usercenter.account.po.UserInfo)
	 */
	@Override
...
//			UserInfo userInfo = getUserInfo();
			...
			if(StringUtils.hasText(ret.getString("ret"))&&ret.getString("ret").equals("00")){
				...
				//TODO 此处为 短信服务特殊初始化
				//初始化短信签名APIkey和secretkey
				MessageSecretKey msgSecret = new MessageSecretKey();
				String secretKey = ApiKeyGenerator.generateSecretKey(11);
				msgSecret.setEnterpriseId(userInfo.getEnterpriseId());
				msgSecret.setSecretKey(secretKey);
				msgSecret.setApiKey(ApiKeyGenerator.generateApiKey(userInfo.getEnterpriseId().toString(), secretKey, null));
				msgSecretkeyService.insertInitValue(msgSecret);
				//返回给前端的结果信息
	...
	/* (non-Javadoc)
	 * @see com.channelsoft.usercenter.business.factory.ProductHandler#initValue(com.channelsoft.usercenter.business.po.EnterpriseProductAjax)
	 */
	@Override
	public void initValue(EnterpriseProductAjax productRela)
